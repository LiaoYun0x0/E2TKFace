...
weight_mask_np = io.imread('uv-data/uv_weight_mask.png').astype(float)#这是忽略脖子的意思
...
def UVLoss(is_foreface=False, is_weighted=False, is_nme=False):
    class TemplateLoss(nn.Module):
        ...
        def forward(self, y_true, y_pred):
            ...
            dist = torch.sqrt(torch.sum((y_true - y_pred) ** 2, 1))#l2loss torch.Size([2, 256, 256])
            if is_weighted:#这是人脸区域赋权
                dist = dist * self.weight_mask
            if is_foreface:
                dist = dist * (self.face_mask * face_mask_mean_fix_rate)#face_mask也是忽略脖子
...
def getLossFunction(loss_func_name='SquareError'):#is_foreface是。is_weighted是忽略脖子。
...
def PRNError(is_2d=False, is_normalized=True, is_foreface=True, is_landmark=False, is_gt_landmark=False,npysavepath=None,i=None):
    def templateError(y_gt, y_fit, bbox=None, landmarks=None,npysavepath=npysavepath,i=i):
        ...
        if is_landmark:
            ...
            if is_gt_landmark:
                ...
                pred = y_pred[uv_kpt[:, 0], uv_kpt[:, 1]]#[68,3]



                np.save(npysavepath + str(2 * i) + "KP.npy", pred[:, 0:2])


                gt[:, 2] = gt[:, 2] - gt[:, 2].mean()
                pred[:, 2] = pred[:, 2] - pred[:, 2].mean()#[68,3] 可以用0:2取x,y
                diff = np.square(gt - pred)
                if is_2d:
                    dist = np.sqrt(np.sum(diff[:, 0:2], axis=-1))
                else:
...
def getErrorFunction(error_func_name='NME',npysavepath=None,i=None):
    if error_func_name == 'nme2d' or error_func_name == 'normalized mean error2d':
        ...
        return PRNError(is_2d=True, is_normalized=True, is_foreface=False, is_landmark=True,npysavepath=npysavepath,i=i)
