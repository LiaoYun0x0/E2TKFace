...
class E2_ResNetFPN_8_2(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # Config
        config = {}
        config['initial_dim'] = 32
        config['block_dims'] = [32, 64, 128]
        config['nbr_rotations'] = 8
        config['e2_same_nbr_filters'] = True
        block = BasicBlock
        ...
        initial_dim = config['initial_dim']#128,从D:\GitHub\se2-loftr\src\config\default.py可知
        block_dims = config['block_dims']#[128, 196, 256],从D:\GitHub\se2-loftr\src\config\default.py可知
        nbr_rotations = config['nbr_rotations']  # 设4，从D:\GitHub\se2-loftr\configs\loftr\outdoor\loftr_ds_e2.py可知

        self.r2_act = gspaces.Rot2dOnR2(N=nbr_rotations)
        self.triv_in_type = enn.FieldType(self.r2_act,
                                          3*[self.r2_act.trivial_repr])
        if config['e2_same_nbr_filters']:#会为True，,从D:\GitHub\se2-loftr\src\config\default.py可知
        ...
        # 3. FPN upsample 这个模块没必要了，第一行第二行留着，其他注释掉
        self.layer3_outconv = conv1x1(reg_repr_blocks[2], reg_repr_blocks[2])#留着，转通道的
    ...
    def forward(self, x):
        x = enn.GeometricTensor(x, self.triv_in_type)#这个一定要留着
        ...
        x3_inv = self.layer3triv(x3_out)#这个一定要留着，通道从regular变成irrev_0
        # x1_inv = x1_out  # trivial repr by design#这个是金字塔特有，注释掉

        #return [x3_inv.tensor, x1_inv.tensor]#这个注释掉，只return第一个
        return x3_inv
