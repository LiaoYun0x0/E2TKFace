...
from lossForKeypoint import getErrorFunction, getLossFunction
...
npySavePath='Q:/GitHub/PRNetOUT/8rotSEtopk/All/image000'
loadModelPath='Q:\GitHub\PRNetOUT\pth\\rot8SEtopk\\net_050.pth'
testPath='Q:/GitHub/PRNet-PyTorch-BU/data/images/AFLW2000-crop'
...
class NetworkManager:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epoch):
            ...
            for i, data in enumerate(train_data_loader):#一个batch
                # prepare data
                x = data[0]#ground truth 是UVpos图
    ...
    def test(self, error_func_list=None, is_visualize=False):
        ...
        with torch.no_grad():
            ...
            for i in range(len(self.test_data)):
                ...
                p = p.squeeze().cpu().numpy().transpose(1, 2, 0) * 280#这好像是posmap
                # cv2.imwrite("Q:\GitHub\PRNetOUT\8rottopk\\net27\\deg6090\\image000"+str(2*i)+".jpg",p)#保存UVposmap

                np.save(npySavePath+str(2*i)+".npy",p)
                ...
                for error_func_name in error_func_list:
                    error_func = getErrorFunction(error_func_name,npysavepath=npySavePath,i=i)
                    error = error_func(gt_y, p, b['Bbox'], b['Kpt'],npysavepath=npySavePath,i=i)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-td', '--trainDataDir', nargs='+', default=['Q:/GitHub/PRNet-PyTorch-BU/data/images/AFLW2000-crop'], type=str, help='training image directories')
    parser.add_argument('-vd', '--valDataDir', nargs='+', default=['Q:/GitHub/PRNet-PyTorch-BU/data/images/AFLW2000-crop'], type=str, help='validation image directories')
    parser.add_argument('-pd', '--testDataDir', nargs='+', default=[testPath], type=str, help='test/predict image directories')
    ...
    parser.add_argument('-test', '--isTest', default=True, type=ast.literal_eval, help='')
    ...
    parser.add_argument('--errorFunction', default=['nme2d', 'nme3d','landmark2d','landmark3d'], nargs='+', type=str)
    parser.add_argument('--loadModelPath', default=loadModelPath,type=str, help='')
