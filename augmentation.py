...
def rotateData_for_show (x, y, angle_range=45, specify_angle=None):
    if specify_angle is None:
        angle = -20
        angle = angle / 180. * np.pi
    else:
...
def prnAugment_torch_show_image(x, y, is_rotate=True):

    if is_rotate:
        x, y = rotateData_for_show(x, y, 90)
        plt.imshow(x)
        plt.axis('off')
        plt.show()
...
if __name__ == '__main__':
    ...
    x = io.imread('Q:\GitHub\PRNet-PyTorch-BU\data/images/AFLW2000-crop/image00039/image00039_cropped.jpg') / 255.
    x = x.astype(np.float32)
    y = np.load('Q:\GitHub\PRNet-PyTorch-BU\data/images/AFLW2000-crop/image00039/image00039_cropped_uv_posmap.npy')
    y = y.astype(np.float32)

    t1 = time.perf_counter()
    for i in range(1):
        xr, yr = prnAugment_torch_show_image(x, y)
