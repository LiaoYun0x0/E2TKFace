    ...
    def __init__(self, in_chans=3, depths=(1, 1, 3, 1), patch_size=4,
        ...
        self.patch_embeds = nn.ModuleList([
            PatchEmbed(patch_size=patch_size if i == 0 else 2,
                       ...
                       is_init_layer=True if i==0 else False)
...
class DaViTFPN(nn.Module):
    def __init__(self,config):
        super(DaViTFPN,self).__init__()
        self.backbone = DaViT(**config)
...
if __name__ == "__main__":
    ...
    model = DaViTFPN(davit_config)
    torch.save(model.state_dict(),"davitfpn.pth")
