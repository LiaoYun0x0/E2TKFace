...
class PositionEncodingSine(nn.Module):
    ...
    def __init__(self, d_model, max_shape=(8, 8), temp_bug_fix=True):
        """
        Args:
        ...
        y_position = torch.ones(max_shape).cumsum(0).float().unsqueeze(0) / 8
        x_position = torch.ones(max_shape).cumsum(1).float().unsqueeze(0) / 8
        if temp_bug_fix:
